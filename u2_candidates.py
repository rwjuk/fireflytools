...
with conn.cursor() as cur:
	...
	cur.execute('select page_namespace, page_title from page left outer join user on (user.user_name = replace(page.page_title,"_"," ")) left outer join categorylinks on (page.page_id = cl_from) where cl_from is null and user.user_name is null and page_namespace=2 and page_title not like "%/%" and page_title not like "%.%.%.%" and page_title not like "%:%:%" and page_is_redirect=False limit {};'.format(limit))
